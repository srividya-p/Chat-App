...
io.on('connection', (socket) => {
    ...
    socket.on('join', (options, callback) => {
        ...
        socket.emit('message', generateMessage('Chat Chan', `Welcome!`))
        socket.broadcast.to(user.room).emit('message', generateMessage('Chat Chan', `${user.username} has joined!`))
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            io.to(user.room).emit('message', generateMessage('Chat Chan', `${user.username} has left!`))
            io.to(user.room).emit('roomData', {
                room: user.room,
                users: getUsersInRoom(user.room)
            })
